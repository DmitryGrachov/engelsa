...
export const buildComparisonsApartments = () => {
    const comparisonIds = new Set(getComparisons());
    /** @type {FilterGroupVariantItem[]} */
    const variants = [];

    if (comparisonIds.size === 0)
        return variants;

    for (const item of dataArray) {
        if (typeof item.id !== 'number' || !comparisonIds.has(item.id))
            continue;
